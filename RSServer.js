...
function sendGameState(socket, ios, mRI, toAll){
  ...
  if (toAll) {                                        //An array of all of the players, names, and card lengths
...
function sitDown(socket, ios, mRI, name){
  ...
  if (added) {
    ...
    sendID(socket, mRI);
  } else {socket.emit("fullTable"); return;}
}
...
function leaveRoom(socket, ios){
  ...
  if (gameRooms[mRI].users.length===0) {
...
function serverClear(num, ios, mRI, clearType){
//Empties the center area and pushes the cards into another player's stack
  var subIos=ios, subNum=num, subRoom=mRI;
  console.log("Server requesting clear.");
  gameRooms[mRI].gL.clearCenter(num, clearType, function() {
    console.log("Running callback");
    sendGameState(subNum, subIos, subRoom, true);
  });
  }
